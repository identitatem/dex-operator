...
func (r *DexClientReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !isOAuth2ClientCreated(dexv1Client.Status.Conditions) {
		...
		// Add the dex client secret's sha256 checksum to the DexClient annotation so that we can detect changes to the Dex Client secret
		var dexClientSecretHash string
		...
		dexClientSecretHash = fmt.Sprintf("%x", h.Sum(nil))
		...
		} else if (hashValue != dexClientSecretHash) {	// Dex client secret has been edited, we must delete and recreate the Oauth2Client
			log.Info("*** The Dex client secret has been edited")
		}
